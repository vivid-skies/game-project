...
@onready var spectrum: AudioEffectInstance = AudioServer.get_bus_effect_instance(3,0)

# @onready var topRightArray = $CircleBase/Right/Top.get_children()

@onready var bottomRightArray: Array[Node] = $CircleBase/Right/Bottom/Container.get_children()
...
func _process(_delta: Variant) -> void:
	...
	for i in range(1,VU_COUNT+1):   
		var hz = i * FREQ_MAX / float(VU_COUNT);
